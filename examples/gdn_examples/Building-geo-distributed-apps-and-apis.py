...
pprint.pprint(
    "Connected to region: {}".format(fabric.localdc(detail=False))
)  # noqa: T203
print(
    "List of regions geo fabric is replicated currently: {}".format(
        fabric.dclist(detail=False)
    )
)
...
print(
    "Register callback to get changes in realtime from collection: {}".format(
        collection_name
    )
)
...
# fabric.create_stream(stream_name, local=False) # geo-replicated stream available in
# all regions.

print("Get all available local streams...")
streams = fabric.persistent_streams(local=True)
pprint.pprint(streams)  # noqa: T203
...
def create_subscriber():
    ...
    for i in range(10):
        ...
        print(
            "Received...Stream:'{}', Body:'{}' ID:'{}'".format(
                msg.topic_name(), msg.data(), msg.message_id()
            )
        )

...
def test_add_index(col):
    ...
    resp.pop("new", None)
    assert resp in col.indexes()
    assert col.delete_index(resp["name"]) is True
...
def test_add_hash_index(col):
    ...
    result = col.add_hash_index(
        fields=fields, unique=False, sparse=True, deduplicate=True
    )
    expected_index = {
        "selectivity": 1,
        "sparse": True,
...
def test_add_skiplist_index(col):
    ...
    # Test if unique set to true in global collection
    with assert_raises(IndexCreateError) as err:
        col.add_skiplist_index(
            fields=fields, unique=True, sparse=True, deduplicate=True
        )
    assert err.value.error_code == 400

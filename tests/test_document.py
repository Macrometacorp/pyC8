...
def test_document_update_many(col, docs):
    ...
    assert (
        str(resp)
        == "[DocumentUpdateError('[HTTP 202][ERR 1227] invalid document type')]"
    )
...
def test_document_replace_many(col, docs):
    ...
    assert (
        str(resp) == "[DocumentReplaceError('[HTTP 202][ERR 1202] document not found')]"
    )
    ...
    assert (
        str(resp)
        == "[DocumentReplaceError('[HTTP 202][ERR 1221] illegal document key')]"
    )
...
def test_document_delete_many(col, docs):
    ...
    assert (
        str(resp) == "[DocumentDeleteError('[HTTP 202][ERR 1202] document not found')]"
    )
...
def test_document_management_via_db(tst_fabric, col):
    ...
    assert (
        tst_fabric.collection(col.name).has(result["_id"], rev=result["_rev"]) is True
    )
    ...
    assert (
        str(err)
        == "<ExceptionInfo DocumentUpdateError('[HTTP 404][ERR 1202] document not found') tblen=5>"
    )
